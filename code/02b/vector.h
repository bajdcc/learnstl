...
public:
    ...
    Vector(const Vector &v) : data((T *) calloc(v.capacity, sizeof(T))), capacity(v.capacity), length(v.length) {
        memcpy(data, v.data, sizeof(T) * length);
    }
    ...
    Vector<T> &operator=(const Vector<T> &v) {
        free(data);
        data = ((T *) calloc(v.capacity, sizeof(T)));
        capacity = v.capacity;
        length = v.length;
        memcpy(data, v.data, sizeof(T) * length);
        return *this;
